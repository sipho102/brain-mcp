...
**`uid` format:** always a UUIDv4, generated by `capture()` at note
creation and never changed afterward — never a timestamp-shaped id like
`YYYYMMDD-HHmm`. This is deliberate: it's what keeps `read_note`'s
short-prefix lookup (≥8 chars) meaningful, which it wouldn't be against a
low-entropy, date-based scheme where every note from the same day shares a
prefix. If a vault's own `CONVENTIONS.md` documents a different `uid`
scheme, that's the vault's documentation that needs updating to match —
this server doesn't special-case it.
